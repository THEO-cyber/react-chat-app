...
const CatchErr = (err:{code?:string}) => {
    ...
    else if(code === "auth/email-already-in-use") toastErr("email already exists");
    else if(code === "auth/wrong-password") toastErr("wrong password");
    else if(code === "auth/requires-recent-login") toastInfo("logout and login before updating your profile");
    ...
    else if (code === "auth/operation-not-allowed") toastErr("Can't change email now")
    else toastErr("An error occured!"); console.log(err, err.code);    
}
